...
start chain=1, node distance=5mm]%
\node[on chain= 1, doc, join=by -latex](plaintext){plain\\text};
\node[on chain=1, proc,
xshift=20mm, align=left](wl){WebLicht\\\parbox{7em}{
...
db](corpusdb){Corpus\\data base\\1 token\\per row};

\node[tm, below right=of corpusdb, tape bend top=0,on chain=1, yshift=-25](tmhead){110100101001};
...
\node[db, below=of corpusdb, left=of tmbottom, yshift=-5mm](examples){examples};
\node[doc, below =of tmbottom, yshift=-8mm](result){tcf\\example\\layer};

\node[doc, above=of plaintext](tei){TEI\\XML};

{[start branch=2]%
  \node[doc, on chain, below=of plaintext](pdf){PDF};
  \node[proc, on chain, right=of pdf](textextract){text\\extr.};
}
